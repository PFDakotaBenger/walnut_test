...
router.get('/api/posts', asyncHandler(async function (req, res) {
    ...
    if (("sortBy" in options) && (sortByParams.indexOf(options.sortBy) === -1)) {
        res.status(400);
        return res.json({"error":"sortBy parameter is invalid"})
        
    } 

    if (("direction" in options) &&(directonParams.indexOf(options.direction) === -1)) {
    ...
    let uniqueArr = [... new Set(data.flat())]
    uniqueArr = uniqueArr.filter((post, index, self) => index === self.findIndex((obj) => (obj.id === post.id)))
    data = uniqueArr
    if (options.sortBy) {
